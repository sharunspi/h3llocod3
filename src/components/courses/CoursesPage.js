...
class CoursesPage extends React.Component{
  ...
  componentDidMount(){
      ...
      if(courses.length ===0){
      actions.loadCourses().catch(err => {console.log(err)}) 
      }
      if(authors.length ===0){
      actions.loadAuthors().catch(err => {console.log(err)}) 
    ...
    .catch(error => { toast.error("Delete Failed "+ error.message, { autoClose:false})})
    }
